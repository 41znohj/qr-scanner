...
// Format MAC address: 2c6f513b4b28 → 2C:6F:51:3B:4B:28
...
function detectQRInImage(src) {
  ...
  img.onload = () => {
    ...
    // Strategy 3: Try aggressive cropping
    if (!result) {
      const cropRegions = [
        { x: 0, y: 0.2, w: 1, h: 0.8 },
    ...
    if (result) {
      ...
      // Hide instructions after successful scan
      if (instructionsDiv) {
...
function parseAssetTag(text) {
  // Format: NF2FHC8ES4TZU6TL111A,2c6f513b4b28,84 (UID + MAC + extra)
  // ONLY format MAC with colons when BOTH UID and MAC are present
  ...
  if (match) {
    ...
    // Format MAC with colons ONLY when UID is also present
    macText.textContent = `${formatMACAddress(macOrRecovery)} (Recovery Code)`;
  ...
  // Keep MAC lowercase WITHOUT colons when NO UID is present
  ...
  if (macMatch) {
    ...
    // Keep original lowercase format when UID is NOT present
